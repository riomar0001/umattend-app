...
const checkOutStudentById = async (
  ...
  try {
    ...
    const checkedOut = await eventServices.checkOutStudentById(
      event_id,
      studentIdNum,
      req.user.id
    );
    ...
    const responseData = {
      event_id: checkedOut.event_id,
      event_name: checkedOut.event.title,
      student_id: checkedOut.student.student_id,
      student_name: checkedOut.student.name,
      checked_out_at: checkedOut.check_out_at,
      checked_out_by: checkedOut.check_out_by,
    };
    ...
    if (NODE_ENV === 'DEVELOPMENT') {
      console.error('Unexpected error checking out student by id', error);
      return HTTPErrorResponse(res, 500, error);
    }
    return HTTPErrorResponse(res, 500, 'Internal server error') as Response;
  }
