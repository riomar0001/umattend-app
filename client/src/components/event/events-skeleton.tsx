  ...
  return (
    <div>
      {[1, 2, 3, 4, 5].map((i, index) => (
        <div key={i} className={`flex items-stretch ${index < 4 ? 'border-border/60 border-b' : ''}`}>
          ...
          <div className="flex w-12 flex-shrink-0 flex-col justify-center py-5 pl-2 sm:w-20 sm:pl-4">
            ...
            <Skeleton className="mt-1.5 ml-auto h-2 w-4" />
            <Skeleton className="mt-1 ml-auto h-2 w-3" />
          </div>

          {/* Hairline */}
          <div className="bg-border/40 mx-3 w-px flex-shrink-0 self-stretch sm:mx-5" />
