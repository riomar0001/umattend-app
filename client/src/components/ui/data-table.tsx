...
interface DataTableProps<TData, TValue> {
  ...
  error?: unknown;
  emptyMessage?: string;
  /** Optional toolbar content rendered between the search bar and columns dropdown */
    ...
    <div className="w-full">
      ...
      <div className="flex flex-wrap items-center gap-3 py-4">
        <div className="relative flex-1 sm:max-w-sm">
          ...
          <Input placeholder={searchPlaceholder} value={inputValue} onChange={(e) => handleSearchChange(e.target.value)} className="pl-10" />
        </div>
        {toolbar}
        <DropdownMenu>
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow>
                <TableCell colSpan={columns.length} className="h-24 text-center text-sm text-red-500">
      ...
      {/* Pagination */}
      <div className="mt-4 flex flex-col items-start justify-between gap-4 text-xs sm:flex-row sm:items-center md:text-sm">
        <p className="text-muted-foreground font-medium">
          {totalRecords === 0 ? 'No records' : `Showing ${(page - 1) * pageSize + 1}–${Math.min(page * pageSize, totalRecords)} of ${totalRecords}`}
        </p>
